...
for node in graph['nodes']:
    ...
    if node_type == 'GlobalInput':
        ...
        for out in node['outputs']:
            for lid in out['links']:
                print('GlobalInput', node['id'],'=>>',links[lid-1][3])

    if node_type == 'GlobalOutput':
        opt = False
        outputs.append(node['properties']['name'])
        for inp in node['inputs']:
            print('GlobalOutput',node['id'],'=>>',links[inp['link']-1][3])
    ...
    for out in node['outputs']:
        ...
        for lid in out['links']:
            print('link id', lid)
            print('opt Node output', node['id'],'=>>',links[lid-1][3])
    # for inp in node['inputs']:
    #     for lid in out['links']:
    #         print('opt Node input', links[lid-1][1],'=>>', node['id'])
            # print('GlobalOutput',node['id'],'=>>',links[inp['link']-1][3])
